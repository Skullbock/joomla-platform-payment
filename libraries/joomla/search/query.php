...
/**
 * Query class for the Joomla search package.
 *
 * @package     Joomla.Platform
 * @subpackage  Search
 * @since       12.1
...
class JSearchQuery
{
	 ...
	 */
	public function __construct($options, JSearch $search, $db = null)
	{
		...
		$this->language	= !empty($options['language']) ? $options['language'] : 'en-GB';
		$this->language	= $this->search->getPrimaryLanguage($this->language);
	...
	public function toURI($base = null)
	{
		...
		if (empty($base)) {
			$base = 'index.php?option=com_search&view=search';
		}
	...
	protected function processStaticTaxonomy($filterId)
	{
		...
		$query->from('#__search_filters AS f');
		...
		// Get a registry object for the filter date options.
		$params = new JRegistry($return->params);
	...
	protected function processDates($date1, $date2, $when1, $when2)
	{
		...
		if ($date1 === JString::strtolower(JText::_('LIB_SEARCH_QUERY_FILTER_TODAY'))) {
		...
		// Check if the date was parsed successfully, and set the date filter appropriately.
		if ($date->toUnix() !== null) {
		...
		// The value of 'today' is a special case that we need to handle.
		if ($date2 === JString::strtolower(JText::_('LIB_SEARCH_QUERY_FILTER_TODAY'))) {
			$today = JFactory::getDate('now', $offset);
	...
	protected function processString($input, $lang, $mode)
	{
		 ...
		 */
		$patterns = array(
			'before'	=> JText::_('LIB_SEARCH_QUERY_FILTER_BEFORE'),
			'after'		=> JText::_('LIB_SEARCH_QUERY_FILTER_AFTER'),
		);

		// Add the taxonomy branch titles to the possible patterns.
		foreach (JSearch::getBranchTitles() as $branch)
		{
			$patterns[$branch] = JString::strtolower(JText::_('LIB_SEARCH_QUERY_FILTER_BRANCH_'.$branch));
		}
			...
			if (preg_match('#'.$pattern.'\s*:\s*'.$suffix.'#mi', $input, $matches)) {
				...
				switch ($modifier)
				{
					...
					case 'after':
						...
						// The value of 'today' is a special case that we need to handle.
						if ($value === JString::strtolower(JText::_('LIB_SEARCH_QUERY_FILTER_TODAY'))) {
							$today = JFactory::getDate('now', $offset);
					...
					default:
						...
						$return = JSearch::getNodeByTitle($modifier, $value);
		...
		$operators = array(
			'AND'	=> JString::strtolower(JText::_('LIB_SEARCH_QUERY_OPERATOR_AND')),
			'OR'	=> JString::strtolower(JText::_('LIB_SEARCH_QUERY_OPERATOR_OR')),
			'NOT'	=> JString::strtolower(JText::_('LIB_SEARCH_QUERY_OPERATOR_NOT')),
		);
		...
		for ($i = 0, $c = count($terms); $i < $c; $i++)
		{
			...
			if (isset($terms[$i+1]) && in_array($terms[$i+1], $operators)) {
				...
				if ($op === 'AND' && isset($terms[$i+2])) {
					...
					$token = JSearch::tokenize($terms[$i], $lang, true);
					...
					$other = JSearch::tokenize($terms[$i+2], $lang, true);
			...
			elseif (isset($terms[$i+1]) && array_search($terms[$i], $operators) === 'OR')
			{
				...
				$other = JSearch::tokenize($terms[$i+1], $lang, true);
				$other = $this->getTokenData($other);
		...
		{
			// Tokenize the phrase.
			$token = JSearch::tokenize($phrases[$i], $lang, true);
			$token = $this->getTokenData($token);
		...
		if (!empty($terms))
		{
			// Tokenize the terms.
			$terms	= implode(' ', $terms);
			$tokens = JSearch::tokenize($terms, $lang, false);
	 ...
	 *
	 * @param   object  $token  A JSearchToken object.
	 *
	 * @return  object  A JSearchToken object.
	 *
	 * @since   12.1
	 * @throws  Exception on database error.
	 */
	protected function getTokenData($token)
	{
		...
		if ($token->phrase) {
			...
			$sql->where('t.term = '.$this->db->quote($token->term));
			...
			$sub = clone($sql);
			$sub->clear('where');
			$sub->where('t.stem = '.$this->db->quote($token->stem));
		...
		if (empty($token->matches))
		{
			...
			$sql->from('#__search_terms AS t');
			$sql->where('t.soundex = SOUNDEX('.$this->db->quote($token->term).')');
